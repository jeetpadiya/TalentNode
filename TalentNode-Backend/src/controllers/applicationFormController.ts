...
import { applicationFormSchema, customQuestionSchema } from "../validations/applicationFormSchemas.js";
import {
...
const createCustomQuestion = async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ success: false, message: "Unauthorized" });
    ...
    const parsedBody = customQuestionSchema.safeParse(req.body);
    if (!parsedBody.success) {
      return res.status(400).json({
        success: false,
        message: parsedBody.error.issues[0]?.message ?? "Invalid custom question",
      });
    }
    ...
    const currentForm = job.applicationForm ?? {
      basicInfo: { phone: "Hidden", location: "Hidden" },
      links: [],
      fileUploads: [],
      customQuestions: [],
    ...
    const question = {
      ...parsedBody.data,
      key: parsedBody.data.key.trim().toLowerCase(),
      question: parsedBody.data.question.trim(),
      options: parsedBody.data.options.map((option) => option.trim()).filter(Boolean),
    };

    const duplicate = currentForm.customQuestions?.some(
      (item) => item.key === question.key,
    );
    if (duplicate) {
      return res.status(409).json({
        success: false,
        message: "A custom question with this key already exists",
      });
    }

    const nextForm = {
      ...currentForm,
      customQuestions: [...(currentForm.customQuestions ?? []), question],
    };

    const parsedForm = applicationFormSchema.safeParse(nextForm);
    if (!parsedForm.success) {
      return res.status(400).json({
        success: false,
        message: parsedForm.error.issues[0]?.message ?? "Invalid application form",
      });
    }

    job.applicationForm = parsedForm.data;
    await job.save();
...
const updateCustomQuestion = async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ success: false, message: "Unauthorized" });
    ...
    const jobId = getParamValue(req.params.jobId);
    const questionKey = getParamValue(req.params.questionKey);
    if (!jobId || !mongoose.isValidObjectId(jobId) || !questionKey) {
    ...
    const existingIndex = (currentForm.customQuestions ?? []).findIndex(
      (item) => item.key === questionKey,
    );
    if (existingIndex === -1) {
      return res.status(404).json({ success: false, message: "Custom question not found" });
    }
    ...
    const duplicate = (currentForm.customQuestions ?? []).some(
      (item, index) => index !== existingIndex && item.key === nextQuestion.key,
    );
    if (duplicate) {
      return res.status(409).json({
        success: false,
    ...
    const customQuestions = [...(currentForm.customQuestions ?? [])];
    customQuestions[existingIndex] = nextQuestion;
...
const deleteCustomQuestion = async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ success: false, message: "Unauthorized" });
    ...
    const customQuestions = (currentForm.customQuestions ?? []).filter(
      (item) => item.key !== questionKey,
    );

    if (customQuestions.length === (currentForm.customQuestions ?? []).length) {
      return res.status(404).json({ success: false, message: "Custom question not found" });
    }
